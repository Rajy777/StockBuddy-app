...
export const searchStocks = cache(async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
        ...
        if (!trimmed) {
            ...
            const profiles = await Promise.all(
                top.map(async (sym) => {
                    try {
                        ...
                        return { sym, profile: null } as { sym: string; profile: FinnhubProfile2Response | null };
                    }
                })
            );
            ...
            results = (Array.isArray(data?.result) ? data.result : []) as FinnhubSearchResultWithExchange[];
        }
